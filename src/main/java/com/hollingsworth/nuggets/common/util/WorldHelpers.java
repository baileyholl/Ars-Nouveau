...
public class WorldHelpers {
    ...
    public static StructureTemplate getStructure(Level level, BlockPos first, BlockPos second){
        ...
        // 1.21.11: fillFromWorld last arg changed from @Nullable Block to List<Block>
    ...
    public static boolean isOfWorldType(@NotNull final Level world, @NotNull final ResourceKey<DimensionType> type) {
        ...
        Identifier loc = dynRegistries.lookup(Registries.DIMENSION_TYPE).map(r -> r.getKey(world.dimensionType())).orElse(null);
        if (loc == null) {
            // 1.21.11: DimensionType.effectsLocation() removed — registry lookup is the only reliable check
            return false;
        }
    ...
    public static boolean isPeaceful(@NotNull final Level world) {
        // getGameRules() moved to ServerLevel in 1.21.11; LevelData no longer exposes it
        if (world instanceof ServerLevel serverLevel) {
            return !serverLevel.getGameRules().get(GameRules.SPAWN_MOBS) || world.getDifficulty().equals(Difficulty.PEACEFUL);
        }

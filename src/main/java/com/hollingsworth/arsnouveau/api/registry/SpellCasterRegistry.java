...
public class SpellCasterRegistry {
    ...
    public static @Nullable AbstractCaster<?> from(ItemStack stack){
        return SPELL_CASTER_TYPES.getOptional(stack.getItem().builtInRegistryHolder().key().location()).orElse((s) -> s.get(DataComponentRegistry.SPELL_CASTER)).getSpellCaster(stack);
    }

    public static boolean hasCaster(ItemStack stack) {
        return SPELL_CASTER_TYPES.containsKey(stack.getItem().builtInRegistryHolder().key().location());
    }

    public static void register(ItemLike itemLike, ItemCasterProvider provider) {
        Registry.registerForHolder(SPELL_CASTER_TYPES, itemLike.asItem().builtInRegistryHolder().key().location(), provider);
    }

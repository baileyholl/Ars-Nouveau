...
public class ParticleColorRegistry {
    ...
    private static ConcurrentHashMap<Identifier, IParticleProvider> MAP = new ConcurrentHashMap<>();
    ...
    public static void register(Identifier id, IParticleProvider factory) {
        MAP.put(id, factory);
    }
    ...
    public static ParticleColor from(@Nullable CompoundTag compoundTag) {
        ...
        return MAP.getOrDefault(Identifier.tryParse(compoundTag.getStringOr("type", "")), DEFAULT).create(compoundTag);
    }

    public static ParticleColor from(Identifier location, int r, int g, int b) {

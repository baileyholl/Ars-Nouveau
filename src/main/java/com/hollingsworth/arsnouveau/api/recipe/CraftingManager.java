...
public class CraftingManager {
    ...
    public void write(ValueOutput tag) {
        tag.store("output_stack", ItemStack.OPTIONAL_CODEC, outputStack);
    ...
    public void read(ValueInput tag) {
        outputStack = tag.read("output_stack", ItemStack.OPTIONAL_CODEC).orElse(ItemStack.EMPTY);
        neededItems = NBTUtil.readItems(tag, "progress");
        remainingItems = NBTUtil.readItems(tag, "refund");
    }

    public static CraftingManager fromTag(ValueInput tag) {
        CraftingManager craftingManager = tag.getBooleanOr("isPotion", false) ? new PotionCraftingManager() : new CraftingManager();

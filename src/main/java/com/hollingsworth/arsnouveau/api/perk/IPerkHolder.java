...
public interface IPerkHolder<T> {
    ...
    default void appendPerkTooltip(Consumer<Component> tooltip, ItemStack stack) {

        for (PerkInstance perkInstance : getPerkInstances(stack)) {
            IPerk perk = perkInstance.getPerk();
            Identifier location = perk.getRegistryName();
            tooltip.accept(Component.literal(Component.translatable("item." + location.getNamespace() + "." + location.getPath()).getString()
        ...
        for (int i = 0; i < missing; i++) {
            ...
            tooltip.accept(Component.literal(Component.translatable("ars_nouveau.empty").getString() + " " + RomanNumber.toRoman(slot.value())).withStyle(ChatFormatting.RED).withStyle(ChatFormatting.ITALIC));
        }
    }

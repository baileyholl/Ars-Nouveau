...
public class GenericTileRenderer<T extends BlockEntity & GeoAnimatable> extends GeoBlockRenderer<T, ArsBlockEntityRenderState> {
    ...
    public static Supplier<GeoItemRenderer<?>> getISTER(String loc) {
        return () -> new GenericItemBlockRenderer(new GenericModel<>(loc));
    }

    public static Supplier<GeoItemRenderer<?>> getISTER(GeoModel model) {

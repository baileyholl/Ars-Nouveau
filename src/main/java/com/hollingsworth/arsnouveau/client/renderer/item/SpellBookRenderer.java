...
public class SpellBookRenderer extends GeoItemRenderer<SpellBook> {
    ...
    protected void renderInGui(ItemDisplayContext transformType, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        MultiBufferSource.BufferSource defaultBufferSource =  Minecraft.getInstance().renderBuffers().bufferSource();
        ...
        this.defaultRenderItem(poseStack, this.animatable, defaultBufferSource, renderType, buffer, 0.0F, Minecraft.getInstance().getFrameTime(), fullbright);
    ...
    public void defaultRenderItem(PoseStack poseStack, SpellBook animatable, MultiBufferSource bufferSource, @Nullable RenderType renderType, @Nullable VertexConsumer buffer, float yaw, float partialTick, int packedLight) {
        poseStack.pushPose();
        packedLight = 15728880;
//        poseStack.mulPose(Axis.YP.rotation(-90));
//        poseStack.translate(0, 3, 0);
        Color renderColor = Color.WHITE;// this.getRenderColor(animatable, partialTick, packedLight);

...
public class PortalTileRenderer<T extends PortalTile> extends TileEntityRenderer<T> {
    ...
    public void render(T tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        this.renderCube(tileEntityIn, f, tileEntityIn.getWorld().getCelestialAngle(partialTicks), matrix4f, bufferIn.getBuffer(RENDER_TYPES.get(0)));

        for(int j = 1; j < i; ++j) {
            this.renderCube(tileEntityIn, f, tileEntityIn.getWorld().getCelestialAngle(partialTicks) / (float)(12.0f  - j), matrix4f, bufferIn.getBuffer(RENDER_TYPES.get(j)));
        }

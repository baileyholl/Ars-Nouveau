...
public class AddTomeCommand {
    private static final SuggestionProvider<CommandSourceStack> sugg = (ctx, builder) -> SharedSuggestionProvider.suggestResource(ANRegistries.CASTER_TOMES.keySet(), builder);
    ...
    private static int spawnTome(CommandSourceStack source, String tome) {
        var data = ANRegistries.CASTER_TOMES.entrySet().stream().filter(e -> e.getKey().location().toString().equals(tome)).findFirst();

...
public class EntityBookwyrm extends FlyingMob implements IPickupResponder, IPlaceBlockResponder, IDispellable, ITooltipProvider, IWandable, IInteractResponder, IAnimatable {
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if(stack.getItem() instanceof SpellParchment){
            ...
            if(new EntitySpellResolver(new SpellContext(spell, this)).canCast(this)) {
                ...
                player.sendMessage(new TranslatableComponent("ars_nouveau.whelp.spell_set"), Util.NIL_UUID);
            } else{
                player.sendMessage(new TranslatableComponent("ars_nouveau.whelp.invalid"), Util.NIL_UUID);
            }
            return InteractionResult.SUCCESS;
        }else if(stack.isEmpty()){
            if(spellRecipe == null || spellRecipe.recipe.size() == 0){
                player.sendMessage(new TranslatableComponent("ars_nouveau.whelp.desc"), Util.NIL_UUID);
            }else
                player.sendMessage(new TranslatableComponent("ars_nouveau.whelp.casting", spellRecipe.getDisplayString()), Util.NIL_UUID);
            return InteractionResult.SUCCESS;
        }

        if(!stack.isEmpty()){
            setHeldStack(new ItemStack(stack.getItem()));
            player.sendMessage(new TranslatableComponent("ars_nouveau.whelp.spell_item", stack.getItem().getName(stack).getString()), Util.NIL_UUID);
        }
    ...
    public void onWanded(Player playerEntity) {
        ...
        PortUtil.sendMessage(playerEntity, new TranslatableComponent("ars_nouveau.whelp.strict_mode", this.entityData.get(STRICT_MODE)));
    }

...
public class FlyHelper extends MovementController {
    ...
    public void tick() {
        ...
        if (mob.collidedHorizontally) {
            mob.rotationYaw += 180.0F;

        }

        float f = (float)(mob.orbitOffset.x - mob.getPosX());
        float f1 = (float)(mob.orbitOffset.y - mob.getPosY());
        float f2 = (float)(mob.orbitOffset.z - mob.getPosZ());
        ...
        float f5 = MathHelper.wrapDegrees(mob.rotationYaw + 90.0F);
        float f6 = MathHelper.wrapDegrees(f4 * (180F / (float)Math.PI));
        mob.rotationYaw = MathHelper.approachDegrees(f5, f6, 4.0F) - 90.0F;
        mob.renderYawOffset = mob.rotationYaw;
        if (MathHelper.degreesDifferenceAbs(f3, mob.rotationYaw) < 3.0F) {
        ...
        mob.rotationPitch = f7;
        float f8 = mob.rotationYaw + 90.0F;
        ...
        mob.setMotion(vector3d.add((new Vector3d(d3, d5, d4)).subtract(vector3d).scale(0.2D)));
    }

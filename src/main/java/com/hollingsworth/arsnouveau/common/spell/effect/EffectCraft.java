...
public class EffectCraft extends AbstractEffect {
    ...
    public void onResolve(HitResult rayTraceResult, Level world, @NotNull LivingEntity shooter, SpellStats spellStats, SpellContext spellContext, SpellResolver resolver) {
        if (shooter instanceof Player playerEntity && isNotFakePlayer(shooter)) {
            playerEntity.openMenu(new SimpleMenuProvider((id, inventory, player) -> new CustomWorkbench(id, inventory, ContainerLevelAccess.create(player.level(), player.blockPosition())), CONTAINER_NAME));
        }
    }
